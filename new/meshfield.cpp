...
static float		g_wave_amplitude = 30.0f;	// 波の振幅

static float		g_wave_hight  = -30.0f;		// 波の高さ補正
bool				WaveType = false;
//=============================================================================
// 初期化処理
//=============================================================================
...
void UpdateMeshField(void)
{
	...
	for (int z = 0; z < g_nNumBlockZField; z++)
	{
		for (int x = 0; x < g_nNumBlockXField; x++)
		{
			...
			if (WaveType)
			{
				g_Vertex[z * (g_nNumBlockXField + 1) + x].Position.y = sin(-g_Time * g_wave_frequency + len * g_wave_correction) * g_wave_amplitude - g_wave_hight;
			}
			else
			{
			g_Vertex[z * (g_nNumBlockXField + 1) + x].Position.y = 0.0f;
